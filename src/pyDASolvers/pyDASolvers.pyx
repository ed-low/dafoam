...
cdef extern from "DASolvers.H" namespace "Foam":
    cppclass DASolvers:
        ...
        void getGlobalIndexLists(List[int]&, List[int]&, List[int]&, List[int]&, List[int]&)
...
cdef class pyDASolvers:
    ...
    def getGlobalIndexLists(self):
        ...
        cdef List[int] cellVectorGlobalIdx
        cdef List[int] faceGlobalIdx

        self._thisptr.getGlobalIndexLists(adjStateGlobalIdx, pointGlobalIdx, cellGlobalIdx, cellVectorGlobalIdx, faceGlobalIdx)
        ...
        py_cell_vector_idx  = np.array([cellVectorGlobalIdx[i] for i in range(cellVectorGlobalIdx.size())], dtype=np.int32)
        py_face_idx         = np.array([faceGlobalIdx[i] for i in range(faceGlobalIdx.size())], dtype=np.int32)

        return py_adjoint_idx, py_point_idx, py_cell_idx, py_cell_vector_idx, py_face_idx
